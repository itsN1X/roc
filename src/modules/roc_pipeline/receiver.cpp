...
void Receiver::run() {
    roc_log(LOG_DEBUG, "receiver: starting thread: output_latency=%u session_latency=%u",
    ...
    roc_log(LOG_DEBUG, "receiver: finishing thread");
...
bool Receiver::tick() {
    ...
    if (!buffer) {
        roc_log(LOG_ERROR, "receiver: can't compose sample buffer");
        return false;
    }
